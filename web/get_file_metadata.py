...
def extract_metadata(pdf_metadata, debug=True):
    ...
    metadata_final['File name'] = get_key_val('resourceName', pdf_metadata).replace(
                "b'", '').replace(".pdf'", "")  # create contenders for titles
    metadata_final['charsPerPage'] = get_key_val('pdf:charsPerPage', pdf_metadata)
    ...
    metadata_final['doc_type'] = doc_type #returns 'Report' as doc_type maninly, might be discarded in future

  	#get the dates for document creation
    creation_date_list = ['Creation-Date', 'meta:creation-date','pdf:docinfo:created','created','dcterms:created']
    doc_created_date = get_val_for_any_in_key_list(creation_date_list, pdf_metadata)
    ...
    doc_title_list = ['Subject','meta:keyword', 'pdf:docinfo:keywords', 'dc:subject']
